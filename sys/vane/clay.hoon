...
++  de                                                  ::  per desk
  ...
  ++  take-patch
    ...
    ?.  ?=([%complete %success *] res)
      =.  dok  ~
      ::  TODO: ADD BACK REAL ERROR PRINTING.
      (print-to-dill '!' %rose [" " "" ""] leaf+"clay patch failed" ~)
    ::  ~&  %editing
    =+  ^-  sim/(list (pair path misu))
        ?~  dok
          ~|(%no-changes !!)
  ...
  ++  take-ergo
    ...
    ^+  +>
    ?.  ?=([%complete %success *] res)
      (print-to-dill '!' %rose [" " "" ""] leaf+"clay ergo failed" ~)
    ?~  hez  ~|(%no-sync-duct !!)
  ...
  ++  take-foreign-x
    ...
    ?.  ?=([%complete %success *] res)
      ...
      ::  ~>  %mean.|.(%*(. >[%plop-fail %why]< |1.+>))
      !!
    ::  ?>  ?=(@ p.p.res)
    wake(haw.u.ref (~(put by haw.u.ref) [car cas pax] `(result-to-cage:ford-api build-result.res)))
  ::
...
++  take                                              ::  accept response
  ...
  ^+  [p=*(list move) q=..^$]
  ?:  ?=({$merge @ @ @ @ @ $~} tea)
    ::  TODO: the issue is that we are conditioning on $made, which could be %t
    ?>  ?=(?($writ $made) +<.q.hin)
    =+  our=(slav %p i.t.tea)
    =*  syd  i.t.t.tea
